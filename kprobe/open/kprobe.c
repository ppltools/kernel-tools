...
static char symbol[MAX_SYMBOL_LEN] = "vfs_open";
...
{
    struct path *path = regs->di;
    struct dentry *dentry = path->dentry;
    struct inode *inode = dentry->d_inode;
    if( inode->i_ino == 3849851351)
    {
        printk("tsk %s pid %d open file %ls!\n",current->comm,current->pid,dentry->d_name.name);
    }
    return 0;
